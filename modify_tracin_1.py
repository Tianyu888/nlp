...
checkpoint = torch.load('./checkpoints/checkpoint_{}_epoch.pkl'.format(5))
model.load_state_dict(checkpoint['model_state_dict'])
optimizer.load_state_dict(checkpoint["optimizer_state_dict"])
epoch = checkpoint['epoch']
print("epoch_{} accuracy: ".format(5), evaluate(test_dataloader))
...
for idx in valid_accu_log:
    if sum(valid_accu_log[idx][:3]) == 0:
        ...
        easy.append(idx)
select_valid_list = easy[:4] + median_easy[:4] + median_hard[:4] + hard[:4]
...
#     tmp_set = set()
#     with torch.no_grad():
#         for idx, (label, text, offsets) in enumerate(valid_list):
#             total_acc, total_count = 0, 0
#             predicted_label = model(text, offsets)
#             total_acc += (predicted_label.argmax(1) == label).sum().item()
#             total_count += label.size(0)
#             acc = float(total_acc) / total_count
#             acc_dict[idx] = acc
#     acc_dict = sorted(acc_dict.items(), key=lambda d: d[1],reverse=True)
#     cls1, cls2, cls3, cls4 = 0, 0, 0, 0
#     acc_list_new = []
#     for idx, (idx2, acc) in enumerate(acc_dict):
#         (label_example_valid, text_example_valid, offset_example_valid) = valid_list[idx2]
#         if label_example_valid == 0 and cls1 <= 2:
#             cls1 += 1
#             acc_list_new.append((idx2, acc))
#         elif label_example_valid == 1 and cls2 <= 2:
#             cls2 += 1
#             acc_list_new.append((idx2, acc))
#         elif label_example_valid == 2 and cls3 <= 2:
#             cls3 += 1
#             acc_list_new.append((idx2, acc))
#         elif label_example_valid == 3 and cls4 <= 2:
#             cls4 += 1
#             acc_list_new.append((idx2, acc))
#         if cls1 == 3 and cls2 == 3 and cls3 == 3 and cls4 == 3:
#             break
#
    #     for idx, (idx2, acc) in enumerate(acc_list_new):
    #         (label_example_valid, text_example_valid, offset_example_valid) = valid_list[idx2]
    #         curr_trc = list()
    #         loss_function = torch.nn.CrossEntropyLoss()
    #         logits_test = model(text_example_valid.to(device), offset_example_valid.to(device))
    ...
    #
    #         curr_trc.sort(key= lambda x: x[1], reverse=True)
    #         for idx4, score in curr_trc[:2500]:
    #             tmp_set.add(idx4)
#     for i in tmp_set:
#         new_train_list.append(train_list[i])
#     train_list = new_train_list
#     print("New training size: {} at epoch {} ".format(len(tmp_set), epoch))
#     epoch += 1
#     # train_list = new_train_list
#     train(train_list)
#     accu_val = evaluate(test_dataloader)
